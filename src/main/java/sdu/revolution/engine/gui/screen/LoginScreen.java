package sdu.revolution.engine.gui.screen;
...
import static sdu.revolution.engine.gui.GuiLibrary.TRANSITION_BACKGROUND;
import static sdu.revolution.engine.gui.GuiLibrary.TRANSITION_FONT;
...
public class LoginScreen extends Panel {
    ...
    public void close() {
        GuiLibrary.runTransition(this, 0, 0, 1000, TRANSITION_BACKGROUND | TRANSITION_FONT);
        new Thread(() -> {
            try {
                Thread.sleep(1500);
            ...
            if (event.getAction() == MouseClickEvent.MouseClickAction.RELEASE) {
                try {
                    if (login(textInput.getTextState().getText())) {
                        ...
                        new Thread(() -> {
                            try {
                                Label fail = new Label("Succeed to Login. Prepare to enter the battlefield.", 0, GuiLibrary.height - 450, GuiLibrary.width, 50);
                                fail.getStyle().setFont("Impact");
